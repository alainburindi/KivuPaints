...
router
  ...
  .get('/dashboard', ensureAuthenticated, (req, res) => {
    const getSales =
      'SELECT s.quantity, s.created_at as date, p.name, p.price, u.name as unity FROM Sales as s INNER JOIN Products as p ON s.product_id = p.id INNER JOIN unities as u ON p.unity_id = u.id ORDER BY s.id DESC';
